...
void PhysicalProcess::handleMessage(cMessage *msg) {
	...
	double returnValue;
	returnValue = calculateReturnValue(receivedMsg->getXCoor(), receivedMsg->getYCoor(), receivedMsg->sendingTime());
...
double PhysicalProcess::calculateReturnValue(const double & x_coo, const double & y_coo, const simtime_t & stime)
{
	double retVal;
	int i, j;
