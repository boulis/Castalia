...
void Mac802154Module::fromRadioLayer(cPacket * pkt, double rssi, double lqi) {
	...
	case MAC_802154_BEACON_PACKET: {
	    ...
	    for (int i = 0; i < (int)rcvPacket->getGTSlistArraySize(); i++) {
		    ...
		    GTSstart = (rcvPacket->getGTSlist(i).start - 1) * baseSlotDuration * ( 1 << frameOrder) * symbolLen;
		    GTSend = GTSstart + rcvPacket->getGTSlist(i).length * baseSlotDuration * ( 1 << frameOrder) * symbolLen;
		    GTSlength = GTSend - GTSstart;
		    trace() << "GTS slot from " << getClock() + GTSstart << " to " << getClock() + GTSend;
	        }
	    }
	    ...
	    if (enableCAP && GTSstart != CAPend) setTimer(START_SLEEPING,CAPend);
	    if (GTSstart != 0 && GTSstart != CAPend) setTimer(GTS_START,GTSstart);
	...
	case MAC_802154_ASSOCIATE_PACKET: {
	    ...
	    if (rcvPacket->getPANid() != SELF_MAC_ADDRESS) break;
	...
	case MAC_802154_ACK_PACKET: {
	    if (rcvPacket->getDstID() != SELF_MAC_ADDRESS) break;
	...
	default: {
	    trace() << "WARNING: unknown packet type received " << rcvPacket->getMac802154PacketType() << " [" << rcvPacket->getName() << "]";
	}
    }
}
...
void Mac802154Module::continueCSMACA() {
	...
	simtime_t backoffBoundary = (ceil((getClock() - currentFrameStart)/(unitBackoffPeriod*symbolLen)) -
				    (getClock() - currentFrameStart)/(unitBackoffPeriod*symbolLen)) *
...
void Mac802154Module::transmitNextPacket() {
    ...
    if (currentFrameStart + CAPend > getClock() + txTime) {
        // still in CAP period of the frame
	if (!enableCAP && nextPacket->getMac802154PacketType() == MAC_802154_DATA_PACKET) allowTx = 0;
    } else if (requestGTS == 0 || GTSstart == 0) {
        // not in CAP period and not in GTS, no transmissions possible
        allowTx = 0;
