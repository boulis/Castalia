...
void TunableMacModule::initialize() 
{
	...
	if ((dutyCycle > 0) && (dutyCycle < 1))
	{
		...
		sleepInterval = listenInterval * ((1-dutyCycle)/dutyCycle);
	}
	else 
		sleepInterval = -1;
			...
			if(!BUFFER_IS_FULL)
			{
				...
				{		
					...
					{	
							...
							if(encapsulateAppPacket(rcvAppDataPacket, dataFrame))
							{
							...
							else
							{
								...
								dataFrame = NULL;
								EV << "\n[Mac]:\n WARNING: Application sent to MAC an oversized packet...packet dropped!!";
							}
		...
		{	
			 ...
			 {	
			 	 ...
			 	 if ( (macState == MAC_STATE_DEFAULT) || (macState == MAC_STATE_TX) ) 
			 	 {
					...
					double dataTXtime = dataFrame->byteLength() * 8 / (1000 * radioDataRate);
					//schedule a self-msg to update the node's state later
					scheduleAt(simTime()+ beacon_offset + dataTXtime + epsilon, new MAC_ControlMessage("check schedTXBuffer buffer", MAC_SELF_CHECK_TX_BUFFER));
			 ...
			 else //MAC buffer is empty
			 {
				...
				if ((dutyCycle > 0) && (dutyCycle < 1)) 
					//setRadioState(MAC_2_RADIO_ENTER_SLEEP, DRIFTED_TIME(listenInterval));
					scheduleAt(simTime() + DRIFTED_TIME(listenInterval), new MAC_ControlMessage("put_radio_to_sleep", MAC_SELF_SET_RADIO_SLEEP));
			 }
		...
		case MAC_FRAME:
		{
			...
			if( (macState == MAC_STATE_DEFAULT) || (macState == MAC_STATE_EXPECTING_RX) || (macState == MAC_STATE_CARRIER_SENSING) )   
			{
				...
				{	
					...
					// if(rcvFrame->getHeader().destID != self) ==> drop
					// we can just forward it to the Application or to Routing module so that the decide whether or not to keep the packet
					...
					//No need to create a new message because of the decapsulation: appDataPacket = new App_GenericDataPacket("Application Packet MAC->Application", APP_DATA_PACKET);
					
					//decaspulate the received MAC frame and create a valid App_GenericDataPacket
			...
			if(macState == MAC_STATE_CARRIER_SENSING)
			{
				...
				switch(backoffType)
				{
					...
					case BACKOFF_EXPONENTIAL:{
							backoffTimer = backoffBaseValue * pow(2, backoffTimes); 
							break;
							}
				}
...
void TunableMacModule::finish()
{
	...
	{	
		macMsg = popTxBuffer();//check_and_cast<MAC_GenericFrame*>(schedTXBuffer.pop_back());
		//appMsg = check_and_cast<App_GenericDataPacket *>(macMsg->decapsulate());
		//EV << "\n--Value(Mac Cleanup): " << macMsg->getHeader().srcID << "\n";
	...
	if(printDropped)
		EV << "\nNode[" << self << "]   *  MAC --> dropped Packets at MAC module:\t\t\t (" << droppedBeacons << ") beacons   and   (" << droppedDataFrames << ") data frames";
...
{
	printDropped = par("printDroppedStatistics");
	...
	backoffBaseValue = ((double)par("backoffBaseValue"))/1000;			// just convert msecs to secs
}




void TunableMacModule::setRadioState(MAC_ContorlMessageType typeID, double delay)
{
...
void TunableMacModule::createBeaconDataFrame(MAC_GenericFrame *retFrame)
{
	...
	retFrame->getHeader().destID = BROADCAST_ADDR;
...
int TunableMacModule::encapsulateAppPacket(App_GenericDataPacket *appPacket, MAC_GenericFrame *retFrame)
{
	int totalMsgLen = appPacket->byteLength() + macFrameOverhead;
	if(totalMsgLen > maxMACFrameSize)
		return 0;
	retFrame->setByteLength(macFrameOverhead); //appPacket->byteLength() extra bytes will be added after the encapsulation
	
	retFrame->getHeader().srcID = self;
	retFrame->getHeader().destID = appPacket->getHeader().destID;
