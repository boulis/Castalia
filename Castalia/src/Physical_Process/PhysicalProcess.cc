...
Define_Module(PhysicalProcess);



void PhysicalProcess::initialize() {
...
void PhysicalProcess::handleMessage(cMessage *msg) {
	
	if (msg -> kind() != PHY_SAMPLE_REQ) 
		EV << "\n[PhysicalProcess]:\n Physical Process received message other than SAMPLE_REQ";

	/* 
	 * Find out where the request came from so you can reply to the same node.
	 * cGate *request = msg->arrivalGate();
	 * int rcvGateIndex = request->index()
	 */ 
	...
	switch(inputType)
	{
		...
		default:
		{
			//this case in not really necessary
			opp_error("\nError! Illegal value of parameter \"inputType\" of physicalProcess module.\n"); 
			break;
		}
	}	
	...
	//send(reply, "toNode", rcvGateIndex);
...
void PhysicalProcess::readIniFileParameters(void)
{
		...
		while (valuesTokenizer.hasMoreTokens()) 
		{
			...
			if(isFirstToken)
			{
				...
				if( ( posA != 0) || (posB == string::npos) )
					opp_error("\nError!(A) Illegal parameter format \"directNodeValueAssignment\" of physicalProcess module.\n");
				
				token = token.substr(1, token.size()-1);
				
				if(token.size() < 1)
					opp_error("\nError!(B) Illegal parameter format \"directNodeValueAssignment\" of physicalProcess module.\n");
			...
			else
			{
				...
				if(tokenSize >= 3)
				{
					if( (posA == string::npos) || (posA == 0) || (posA == tokenSize-1) )
						opp_error("\nError! Illegal parameter format \"directNodeValueAssignment\" of physicalProcess module.\n");
	...
	if(inputType == SCENARIO_BASED)	
	{
		...
		if(numSources > 5)
			opp_error("Physical Process parameter \"numSources\" has been initialized with invalid value \"%d\"", numSources);
...
void PhysicalProcess::readScenariosFromIniFile(void)
{
	int i, j;
	...
	for(i=0; i< numSources; i++)  //for every source_X string parameter that contains the scenarios for physical process X
	{
		...
		if(totalValidSourcesScenarios != (numSources))
			opp_error("\nPhysical Process intialization ERROR! The source_X parameters that are initialized in the omnet.ini file are less than the total number of sources.");
	}
	
}


void PhysicalProcess::initHelpStructures(void)
{
	int i;
	...
	/*
	   a source_index for source i is pointing to that snapshot 	of source i that has the max of the times less or equal
	   than the current time e.g if current time = 4.5 and the snapshot times of source i are 0, 1.2, 3, 4.3, 4.7, 5, 8.1
	   then source_index[i] = 3 (is pointing to the snapshot 	with time 4.3)
	*/
...
double PhysicalProcess::calculateScenarioReturnValue(const double & x_coo, const double & y_coo, const simtime_t & stime)
{
	double retVal;
	int i, j;
	...
	if( (stime - time) >= SIMTIME_STEP )
	{
		...
		//update the current state of all sources ==> (x -- y -- value)  with respect to time and the current snapshot od each source
		for(i=0; i<numSources; i++)
		{
			if (source_index[i] >= 0)
			{
				...
				curr_source_state[i].x = sources_snapshots[i][source_index[i]].x + linear_coeff *
											 (sources_snapshots[i][source_index[i]+1].x - sources_snapshots[i][source_index[i]].x);
											 
				curr_source_state[i].y = sources_snapshots[i][source_index[i]].y + linear_coeff *
											 (sources_snapshots[i][source_index[i]+1].y - sources_snapshots[i][source_index[i]].y);
											 
				curr_source_state[i].value = sources_snapshots[i][source_index[i]].value + linear_coeff *
											 (sources_snapshots[i][source_index[i]+1].value - sources_snapshots[i][source_index[i]].value);
			}
		}
	}
