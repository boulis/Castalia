...
channelTemporalModel::channelTemporalModel(const char *file, int rng) {
    ...
    while (getline(f,s)) {
        ...
        while (ct = t.nextToken()) {
    		...
    		//Since we are parsing the body, each valid line here is a pdf description. And valid pdf description can
    		//only start with a time value (either from correlation times list or coherence time)
    		...
    		if (time == coherenceTime) {	//we discovered coherence pdf definition
    			...
    			if (correlationTime[i].time == time) {
    			    if (correlationTime[i].pdfs[index].numOfLayers > 0) {
    			        std::cout << "[TemporalModel] ERROR two pdfs defined for the same ID: " << time << "," << value << 
    			    		"(" << index << ")" << std::endl;
				exit(1);
    			    } else {
		...
		signalVariationResolution = parseFloat(t.nextToken());
    		...
    		if (minSignalVariation >= maxSignalVariation) {
    		    std::cout << "[TemporalModel] ERROR: " << SIGNAL_VAR << " min value cannot exceed max value (format min:res:max)" << std::endl;
    		    exit(1);
    		}
    		//calculate the value of numOfSignalVariationValues based on the min, max and resolution that was provided
    		numOfSignalVariationValues = lround((maxSignalVariation - minSignalVariation)/signalVariationResolution)+1;
    		param_count++;
    		...
    		if (numOfCorrelationTimes == 0) {
    		    std::cout << "[TemporalModel] ERROR: " << CORR_TIME << " expected comma separated list of decreasing numbers" << std::endl;
    		    exit(1);                
    		}
		...
		for (int i = 0; i < numOfCorrelationTimes; i++) {
		    ...
		    //each PDF is initialised with 0 layers. After the parsing all PDFs are checked to have atleast one layer
		    //ptherwise the file is incorrect or incomplete and the model will not work
	...
	for (int j = 0; j < numOfSignalVariationValues; j++) {
		...
		std::cout << "[TemporalModel] ERROR: missing PDF description for " << correlationTime[i].time <<
			"," << (j*signalVariationResolution)+minSignalVariation << std::endl;
		exit(1);
	    }
	}
    }
...
channelTemporalModel::~channelTemporalModel() {
    if (coherencePDF) { delete coherencePDF; }
    if (correlationTime) { 
	for (int i = 0; i < numOfCorrelationTimes; i++) {
	    for (int j = 0; j < numOfSignalVariationValues; j++) {
		for (int k = 0; k < correlationTime[i].pdfs[j].numOfLayers; k++) {
		    PDFLayerType *l = &correlationTime[i].pdfs[j].layers[k];
		    if (l->numOfValues > 0) { delete [] l->values; }
		    if (l->numOfSublayers > 0) { delete [] l->sublayers; }
		}
...
int channelTemporalModel::calculateValueIndex(float f) {
    if (f <= minSignalVariation) { return 0; }
    if (f >= maxSignalVariation) { return numOfSignalVariationValues-1; }
...
float channelTemporalModel::parseFloat(const char * str) {
    ...
    std::cout << "[TemporalModel] ERROR: Unable to parse number value from string '" << str << "'" << std::endl;
    exit(1);
}
...
void channelTemporalModel::parseLayer(const char * str, PDFLayerType * layer) {
    ...
    while (str[0] == ' ') { str++; }
    int tmp = 1;
    while (str[tmp] && (str[tmp] == ' ' || str[tmp] == '\t')) { tmp++; }
    if (str[tmp] == '=') {
	layer->id = str[0];
	str += tmp + 1;
    } else {
    ...
    if (layer->numOfValues > 0) { layer->values = new float[layer->numOfValues]; }
    if (layer->numOfSublayers > 0) { layer->sublayers = new int[layer->numOfSublayers]; }
    sublayers = 0;
    int values = 0;
...
int channelTemporalModel::isValidLayer(const char *c) {
    if (!c[1] && ((c[0] >= 'a' && c[0] <= 'z') || (c[0] >= 'A' && c[0] <= 'Z'))) { return 1; }
    return 0;
}
...
float channelTemporalModel::drawFromPDF(PDFType *pdf) {
	...
	if (index < layer->numOfValues) { return layer->values[index]; }
	else { currentLayer = layer->sublayers[index - layer->numOfValues]; }
    }
    std::cout << "[TemporalModel] ERROR: too deep recursion in channelTemporalModel::drawFromPDF" << std::endl;
    exit(1);
}
