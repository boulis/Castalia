...
void VirtualCastaliaModule::finish() {
	...
	for (int i = 0; i <= hist.numBuckets; i++) EV << " " << hist.buckets[i];
	EV << endl;
    }
    histograms.clear();
}
...
void VirtualCastaliaModule::declareHistogramNocheck(const char* descr, double min, double max, int buckets, int index) {
    ...
    for (int i = 0; i <= buckets; i++) hist.buckets[i] = 0;
    hist.min = min;
    hist.max = max;
...
void VirtualCastaliaModule::collectHistogramNocheck(const char* descr, int index, double value) {
    ...
    if(i != histograms.end()) {
	int num;
	if (value < i->second.min) return;
	else if (value >= i->second.max) num = i->second.numBuckets;
	else num = (int)floor((value - i->second.min)/i->second.cell);
